...
Shader "Unlit/StaticShader"
{
	Properties
    {
        ...
        _ResX ("X Resolution", Float) = 50000
        _ResY ("Y Resolution", Float) = 50000
       
        _ScaleWithZoom("Scale With Cam Distance", Range(0,1)) = 0.0

        _gradDist("Gradient Distance", Float) = 1.5
    ...
    SubShader
    {
        Pass
        {
            ...
            fixed4 frag(float4 screenPos : TEXCOORD0, fragmentInput i) : SV_Target
            {
               ...
               //round the screen coordinates to give it a blocky appearance
				...
				//float2 vFromCenter = (screenPos.x-0.5,screenPos.y-0.5);
               	//float distance = length(vFromCenter);
				float d = distance(float2 (0.5,0.5), screenPos.xy);

				float4 stat2 = lerp(_ColorB,stat,d*_gradDist);

                return fixed4(stat2.xyz, 1.0);
            }
